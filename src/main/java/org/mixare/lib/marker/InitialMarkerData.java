...
/**
 * An object that is responsible for transfering marker data from one plugin to
 * another
 * 
 * This class can be used dataprocessors to fill marker data in it, so that this
 * class can help creating that marker for the data processor:
 * 
 * How to use: -Set the marker name -Fill the marker constructors in the
 * constructor field: title,lat,lng,alt,link,type,color -Add extra primitives
 * with the setExtra(...) field -Add extra non-primitives (but parcelable)
 * objects with the setExtra(ParcelableProperty property)
 * 
 * @author A. Egal
 */
public class InitialMarkerData implements Parcelable {
	...
	public InitialMarkerData(int id, String title, double latitude,
			double longitude, double altitude, String link, int type,
			int colour, String markerType) {
		constr = new Object[9];
		constr[0] = id;
		constr[1] = title;
		constr[2] = latitude;
	...
	public void setExtras(String name, int value) {
		extraPrimitives.put(name, new PrimitiveProperty(primitive.INT.name(),
				value));
	}

	public void setExtras(String name, String value) {
		extraPrimitives.put(name, new PrimitiveProperty(
				primitive.STRING.name(), value));
	}

	public void setExtras(String name, long value) {
		extraPrimitives.put(name, new PrimitiveProperty(primitive.LONG.name(),
				value));
	}
	...
	public void setExtras(String name, float value) {
		extraPrimitives.put(name, new PrimitiveProperty(primitive.FLOAT.name(),
				value));
	}

	public void setExtras(String name, byte[] value) {
		extraPrimitives.put(name, new PrimitiveProperty(primitive.BYTE.name(),
				value));
	}

	public void setExtras(String name, boolean value) {
		extraPrimitives.put(name, new PrimitiveProperty(
				primitive.STRING.name(), String.valueOf(value)));
	}
	...
	public void writeToParcel(Parcel dest, int flags) {
		...
		dest.writeString((String) constr[8]);
	...
	public void readParcel(Parcel in) {
		...
		constr[6] = in.readInt();
		constr[7] = in.readInt();
		constr[8] = in.readString();

		int countPrim = in.readInt();
		for (int i = 0; i < countPrim; i++) {
			extraPrimitives.put(in.readString(), (PrimitiveProperty) in
					.readParcelable(PrimitiveProperty.class.getClassLoader()));
		}

		int countParc = in.readInt();
		for (int i = 0; i < countParc; i++) {
			extraParcelables.put(in.readString(), (ParcelableProperty) in

...
import org.mixare.lib.gui.Object3DException;
...
public class GLText {
	...
	public float drawC(String text, float x, float y) throws Object3DException {
	...
	public float drawCX(String text, float x, float y) throws Object3DException {
		float len = getLength(text); // Get Text Length
	...
	public float getLength(String text) throws Object3DException {
		...
		try {
			for (int i = 0; i < strLen; i++) { // For Each Character in String
												// (Except Last
				int c = (int) text.charAt(i) - CHAR_START; // Calculate
															// Character Index
															// (Offset by First
															// Char in Font)
				len += (charWidths[c] * scaleX); // Add Scaled Character Width
													// to Total Length
			}
		} catch (Exception e) {
			throw new Object3DException(e.getMessage());
		}
